...
- `Neon Database Connection Test` --semantically_similar_to--> `Drizzle Kit Configuration`  [INFERRED] [semantically similar]
  scratch/test-neon.ts → drizzle.config.ts
- `Global CSS Migration to shadcn Theme` --conceptually_related_to--> `Next.js Configuration`  [INFERRED]
  diff.txt → next.config.ts

## Hyperedges (group relationships)
- **Resume Builder Architecture Triad** — resume_builder_architecture, resume_data_schema, template_system [INFERRED 0.75]
- **Dual-Layer Authentication Guard** — proxy_middleware, auth_layout, better_auth [EXTRACTED 1.00]
- **Neon PostgreSQL Infrastructure Setup** — neon_db, drizzle_config, test_neon_script [INFERRED 0.75]
...
- **useRender Pattern** — Badge, BreadcrumbLink [INFERRED]
